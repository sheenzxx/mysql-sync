...
export function buildConfig(cliOptions) {
    const config = {
        ...
        syncObjects: {
            databases: cliOptions.databases ? cliOptions.databases.split(',').map((s) => s.trim()) : undefined,
            tables: cliOptions.tables
                ? cliOptions.tables.split(',').map((s) => {
                    const [db, ...tbl] = s.trim().split('.');
                    return { database: db, table: tbl.join('.') };
                })
                : undefined,
            excludeTables: cliOptions.excludeTables
                ? cliOptions.excludeTables.split(',').map((s) => {
                    const [db, ...tbl] = s.trim().split('.');
                    return { database: db, table: tbl.join('.') };
                })
                : [],
...
function normalizeConfig(config) {
    ...
    config.fullSync = { ...DEFAULTS.fullSync, ...config.fullSync };
    config.incrementalSync = { ...DEFAULTS.incrementalSync, ...config.incrementalSync };
    config.syncObjects = {
    ...
    if (config.fullSync.workerCount < 1)
        config.fullSync.workerCount = 1;
    if (config.fullSync.batchSize < 1)
        config.fullSync.batchSize = 100;
    if (config.incrementalSync.workerCount < 1)
        config.incrementalSync.workerCount = 1;
