...
(function (EventType) {
    ...
    EventType[EventType["START_EVENT_V3"] = 1] = "START_EVENT_V3";
    EventType[EventType["QUERY_EVENT"] = 2] = "QUERY_EVENT";
    EventType[EventType["STOP_EVENT"] = 3] = "STOP_EVENT";
    EventType[EventType["ROTATE_EVENT"] = 4] = "ROTATE_EVENT";
    EventType[EventType["FORMAT_DESCRIPTION_EVENT"] = 15] = "FORMAT_DESCRIPTION_EVENT";
    EventType[EventType["TABLE_MAP_EVENT"] = 19] = "TABLE_MAP_EVENT";
    EventType[EventType["WRITE_ROWS_EVENT_V1"] = 23] = "WRITE_ROWS_EVENT_V1";
    EventType[EventType["UPDATE_ROWS_EVENT_V1"] = 24] = "UPDATE_ROWS_EVENT_V1";
    EventType[EventType["DELETE_ROWS_EVENT_V1"] = 25] = "DELETE_ROWS_EVENT_V1";
    EventType[EventType["WRITE_ROWS_EVENT_V2"] = 30] = "WRITE_ROWS_EVENT_V2";
    EventType[EventType["UPDATE_ROWS_EVENT_V2"] = 31] = "UPDATE_ROWS_EVENT_V2";
    EventType[EventType["DELETE_ROWS_EVENT_V2"] = 32] = "DELETE_ROWS_EVENT_V2";
    EventType[EventType["GTID_EVENT"] = 33] = "GTID_EVENT";
    EventType[EventType["XID_EVENT"] = 16] = "XID_EVENT";
})(EventType || (EventType = {}));
// ---------------------------------------------------------------------------
// MySQL Protocol helpers
// ---------------------------------------------------------------------------
function leReadUint8(buf, off) {
    return buf.readUInt8(off);
}
function leReadUint16(buf, off) {
    return buf.readUInt16LE(off);
}
function leReadUint24(buf, off) {
    return buf.readUInt8(off) | (buf.readUInt8(off + 1) << 8) | (buf.readUInt8(off + 2) << 16);
}
function leReadUint32(buf, off) {
    return buf.readUInt32LE(off);
}
function leReadUint48(buf, off) {
    const lo = buf.readUInt32LE(off);
    const hi = buf.readUInt16LE(off + 4);
    return hi * 0x100000000 + lo;
}
function leReadInt64(buf, off) {
    return buf.readBigInt64LE(off);
}
function writeUint24(buf, off, val) {
    buf[off] = val & 0xff;
    buf[off + 1] = (val >> 8) & 0xff;
    buf[off + 2] = (val >> 16) & 0xff;
}
function leWriteUint32(buf, off, val) {
    buf.writeUInt32LE(val, off);
}
/** Read a length-encoded integer (MySQL packet format) */
function readLenencInt(buf, off) {
...
function readLenencStr(buf, off) {
...
function readFixedLenStr(buf, off, len) {
    const str = buf.toString('utf8', off.value, off.value + len);
    off.value += len;
    return str;
}
...
function parseEventHeader(buf, off) {
    return {
...
export class BinlogClient {
    ...
    onPosition;
    stopped = false;
    binlogStreamActive = false;
    _readPacketResolve = null;
    crc32Enabled = false; // detected from FORMAT_DESCRIPTION_EVENT
    constructor(config, serverId, onRowChange, onPosition) {
        this.config = config;
    ...
    handleEventPayload(payload) {
        ...
        while (this.rowsEventBuf.length >= 19) {
            ...
            if (this.rowsEventBuf.length < hdr.eventLength)
                break;
    ...
    processRotate(buf) {
        ...
        const nextPos = leReadUint48(buf, off.value);
        off.value += 8;
    ...
    processRowsEvent(buf, changeType, version) {
        const off = { value: 19 };
        const tableId = leReadUint48(buf, off.value);
        off.value += 6;
        const flags = leReadUint16(buf, off.value);
        off.value += 2;
        if (version === 2) {
            const extraLen = leReadUint16(buf, off.value);
            off.value += 2;
            if (extraLen > 2) {
        ...
        const bitmapSize = (colCount + 7) >> 3;
        const colPresent1 = buf.subarray(off.value, off.value + bitmapSize);
        off.value += bitmapSize;
        let colPresent2 = null;
        if (changeType === 'update') {
            colPresent2 = buf.subarray(off.value, off.value + bitmapSize);
            off.value += bitmapSize;
        }
    ...
    parseColumnValue(buf, off, colType, meta) {
        switch (colType) {
            case 1 /* ColType.TINY */:
                return buf.readInt8(off.value++);
            case 2 /* ColType.SHORT */:
            case 13 /* ColType.YEAR */: {
                const v = buf.readInt16LE(off.value);
                off.value += 2;
                return v;
            }
            case 9 /* ColType.INT24 */: {
                // 24-bit signed int
            ...
            case 15 /* ColType.VARCHAR */:
            case 253 /* ColType.VAR_STRING */: {
            ...
            case 252 /* ColType.BLOB */:
            case 249 /* ColType.TINY_BLOB */:
            case 250 /* ColType.MEDIUM_BLOB */:
            case 251 /* ColType.LONG_BLOB */: {
            ...
            case 245 /* ColType.JSON */: {
                // Skip JSON for now
                return null;
            }
            case 255 /* ColType.GEOMETRY */: {
                return null;
            }
            default:
                // Unknown type — skip
        ...
        function decimalBinarySize(prec, scale) {
    ...
    async performHandshake() {
        ...
        const connId = leReadUint32(rawGreeting, off.value);
        off.value += 4;
        const authData1 = rawGreeting.subarray(off.value, off.value + 8);
        off.value += 8;
        off.value += 1; // filler
        ...
        const authData2 = rawGreeting.subarray(off.value, off.value + part2Len - 1);
        off.value += part2Len - 1;
        const authPluginData = Buffer.concat([authData1, authData2]);
        // Build client flags (matching mysql2 defaults for MySQL 5.7)
        const clientFlags = (1 << 9) | // PROTOCOL_41 = 512
        ...
        let p = 0;
        payload.writeUInt32LE(clientFlags, p);
        p += 4;
        payload.writeUInt32LE(16777215, p);
        p += 4;
        payload[p] = 45;
        p += 1; // utf8mb4_general_ci
        p += 23; // zeros
        // Username
        payload.write(this.config.user, p, 'ascii');
        p += this.config.user.length;
        payload[p] = 0;
        p += 1;
        // Auth response
        payload[p] = pwHash.length;
        p += 1;
        pwHash.copy(payload, p);
        p += pwHash.length;
        // No database
        payload[p] = 0;
        p += 1;
        // Auth plugin name
        payload.write('mysql_native_password', p, 'ascii');
        p += 21;
        payload[p] = 0;
        p += 1;
    ...
    async registerSlave() {
        ...
        let off = 0;
        buf[off] = COM_REGISTER_SLAVE;
        off += 1;
        leWriteUint32(buf, off, this.serverId);
        off += 4;
    ...
    async dumpBinlog(pos) {
        ...
        let off = 0;
        buf[off] = COM_BINLOG_DUMP;
        off += 1;
        leWriteUint32(buf, off, pos.position);
        off += 4;
        buf.writeUInt16LE(0, off);
        off += 2; // flags
        leWriteUint32(buf, off, this.serverId);
        off += 4;
        buf.write(filename, off, 'ascii');
        off += filenameBytes;
        buf[off] = 0;
        off += 1;
        await this.writePacket(buf.subarray(0, off), 0);
...
// Helpers
// ---------------------------------------------------------------------------
function columnMetadataSize(colType) {
    switch (colType) {
        case 15 /* ColType.VARCHAR */:
        case 254 /* ColType.STRING */:
        case 247 /* ColType.ENUM */:
        case 248 /* ColType.SET */:
        case 16 /* ColType.BIT */:
            return 2;
        case 252 /* ColType.BLOB */:
        case 249 /* ColType.TINY_BLOB */:
...
function blobLenSize(colType) {
    switch (colType) {
        case 249 /* ColType.TINY_BLOB */: return 1;
        case 252 /* ColType.BLOB */: return 2;
        case 250 /* ColType.MEDIUM_BLOB */: return 3;
