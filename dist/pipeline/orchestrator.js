...
export class SyncOrchestrator {
    ...
    async stop() {
        ...
        // Close connections
        if (this.sourcePool)
            await closePool(this.sourcePool).catch(() => { });
        if (this.targetPool)
            await closePool(this.targetPool).catch(() => { });
        logger.info('Sync stopped');
    }
    /** Prepare target schema: create databases and tables */
    async prepareTargetSchema(objects) {
    ...
    async runFullSync(objects) {
        ...
        const tablePool = new WorkerPool(this.config.fullSync.workerCount, async (task) => {
            const batchPool = new WorkerPool(2, // Write 2 batches concurrently
            async (batch) => {
        ...
        tablePool.on('error', (err, task) => {
            logger.error(`Table sync error for ${task.database}.${task.table}:`, err);
        });
    ...
    async startIncrementalSync(syncObjects) {
        ...
        const processor = async (change) => {
            try {
                ...
                switch (change.type) {
                    case 'insert':
                        this.stats.incrementalSync.inserts++;
                        break;
                    case 'update':
                        this.stats.incrementalSync.updates++;
                        break;
                    case 'delete':
                        this.stats.incrementalSync.deletes++;
                        break;
                    case 'ddl':
                        this.stats.incrementalSync.ddl++;
                        break;
                }
            }
            catch (err) {
    ...
    async runSingleTableFullSync(obj) {
        try {
            await this.fullReplayer.initTargetSchema(this.sourcePool, obj.database, obj.table);
            const columns = await getColumns(this.sourcePool, obj.database, obj.table);
            const pks = await getPrimaryKeys(this.sourcePool, obj.database, obj.table);
