...
import type { FullSyncBatch, RowData, SyncConfig } from '../types.js';
...
export class FullSyncReplayer {
   ...
   */
  async writeBatch(batch: FullSyncBatch): Promise<number> {
    ...
    const values = rawValues as mysql.ExecuteValues;
    ...
    try {
      ...
      for (const row of batch.rows) {
        try {
          const rowValues = columns.map(c => row[c] ?? null) as mysql.ExecuteValues;
          await this.pool.execute(
            `INSERT IGNORE INTO \`${database}\`.\`${table}\` (${colList}) VALUES (${placeholders})`,
            rowValues,
          );
          written++;
        } catch (rowErr) {
   ...
   */
  async initTargetSchema(sourcePool: Pool, database: string, table: string): Promise<void> {
    ...
    const [createRows] = await sourcePool.query<mysql.RowDataPacket[]>(
    ...
    let createSQL = (createRows[0] as any)['Create Table'] as string;
   ...
   */
  async prepareForBulkInsert(): Promise<void> {
   ...
   */
  async finalizeBulkInsert(): Promise<void> {
