...
export class BinlogClient {
  ...
  private processQueryEvent(buf: Buffer): void {
    ...
    off += 4; // exec_time
    const schemaLen = buf.readUInt8(off); off += 1;
    off += 2; // error_code
    const statusVarsLen = leReadUint16(buf, off); off += 2;
    ...
    if (!classification) return; // not a DDL statement
...
// ---------------------------------------------------------------------------
// DDL classification helpers
// ---------------------------------------------------------------------------

interface DdlClassification {
  ddlType: string;
  table?: string;
...
function classifyDdl(sql: string): DdlClassification | null {
  ...
  if (rnMatch) {
    ...
    while ((pm = pairRegex.exec(rest)) !== null) {
      ...
      if (oldTable) tableNames.push(oldTable);
      if (newTable) tableNames.push(newTable);
    }
    return {
  ...
  if (/^CREATE\s+DATABASE/i.test(sql)) return { ddlType: 'CREATE DATABASE' };
  if (/^DROP\s+DATABASE/i.test(sql)) return { ddlType: 'DROP DATABASE' };
